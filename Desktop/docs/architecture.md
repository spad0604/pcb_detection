...
4. **Feedback loop** – Failed detections can be re-labeled and re-added to the dataset to continuously improve the model.
...
- **Layers**:
  ...
  - `modules/dashboard/…` contains GetX bindings, controller, and view widgets. Controller orchestrates dataset uploads, training jobs, inference requests, and exposes observable state to multiple panels (Dataset, Training, Inference, Activity Log).
